...
export class PlayerRoutine extends BaseRoutine {
  ...
  public preUpdate() {
    ...
    // 向かせたり歩かせたり
    if (this.inputManager.isPressed(KEY_CODE_Z)) {
      this.character.preUpdateInfo = { type: 'action' }
    }
    else if (direction != null) {
      const [moveX, moveY] = calcMoveXY(direction, 3)
      this.character.preUpdateInfo = { type: 'move', moveX, moveY, nextDirection: direction}
    }
  }
}
...
export class UroUroRoutine extends BaseRoutine {
  ...
  preUpdate() {
    ...
    if (this.isMoving) {
      ...
      this.character.preUpdateInfo = { type: 'move', moveX, moveY, nextDirection: this.character.currentDirection }
    ...
    // 待機中
      ...
      if (this.frameCountToMove <= 0) {
        ...
        this.character.preUpdateInfo = { type: 'move', moveX: 0, moveY: 0, nextDirection: direction }
        this.isMoving = true
      }
    }
  }
