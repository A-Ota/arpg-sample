import { Character } from '@/stages/017/Character'
...
export class Field extends PIXI.Container {
  ...
  public messageEventHandler: ((messageInfos: Array<{ text: string }>) => void) | null = null
  ...
  public update() {
    ...
    fieldCharacters.forEach(fieldCharacter => {
      if (fieldCharacter.character.preUpdateInfo != null) {
        if (fieldCharacter.character.preUpdateInfo.type === 'move') {
        ...
        } else if (fieldCharacter.character.preUpdateInfo.type === 'action') {
          // アクション処理
          console.log('アクション!')
  ...
  private getNearFrontCharacters(fieldCharacter: FieldCharacter): Array<FieldCharacter> {
    ...
    otherFieldCharacters.forEach(otherFieldCharacter => {
      const [x1, y1, x2, y2] = [targetCircle.x, targetCircle.y, otherFieldCharacter.character.hitCircle.x, otherFieldCharacter.character.hitCircle.y]
